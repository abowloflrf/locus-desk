...
- 产品名称为 `Locus Desk`；界面可根据空间保留完整名或使用 `Locus` 简写。
...
当前 Web 端只负责采集 URL、页面标题、用户选区和备注。后端通过持久化任务异步执行 URL 规范化、去重、安全抓取、正文抽取和 HTML 清洗；图片离线化和浏览器插件仍未实现。文本和正文结构离线可读；阅读器保留经过清洗的远程图片链接，但不下载或缓存图片，因此图片仍依赖原站链接可用，不承诺页面视觉还原。
