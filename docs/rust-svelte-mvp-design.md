...
本文设计一个受 Memos 启发、但完全重新实现的自托管个人支持库。首日目标不是复刻 Memos，而是交付两个最常用的闭环：Markdown 快速笔记，以及围绕 Today 的轻量任务管理。
...
首版明确不兼容 Memos API、数据库或导出格式，不实现导入工具。现有 Memos 仓库仅用于产品和实现方式调研，新应用应在独立仓库中开发。为了避免后续扩展时推翻首版，Notes + Today Tasks MVP 从第一天使用稳定的对外 ID、版本化 API 和可独立演进的领域边界。
...
| 层级 | 功能 | 结论 |
| --- | --- | --- |
| Phase 0A 核心 | 登录、Markdown CRUD、时间流、基础搜索、Today 任务 CRUD、单二进制 | 当日必须交付 |
...
- 通过时间流回顾最近记录。
- 通过关键词或标签找回历史内容。
- 用置顶保存近期重点，用归档收起不再活跃的内容。
- 快速添加今天要做的事，勾选完成，并按重点和常规分组浏览。
...
5. 输入 Markdown 并提交，新笔记立即出现在时间流顶部。
6. 用户在右侧 Today 栏添加、完成和整理当天任务。
7. 用户可搜索、按标签筛选、编辑、置顶、归档或删除笔记。
8. 服务重启后，账户、workspace、笔记和任务数据完整保留。
...
- 单所有者用户名和密码登录。
- Markdown 笔记新建、读取、编辑、删除。
- 首页倒序时间流。
- Today 任务的新建、编辑、完成、恢复和删除。
...
- 默认只展示正常状态的笔记，归档内容进入独立页面。
- 默认排序为置顶优先，其余按创建时间倒序。
- 编辑采用原位编辑或紧邻卡片的编辑状态，避免跳转到复杂详情页。
- 删除必须二次确认；归档和置顶可以直接操作并提供即时反馈。
- 创建成功后清空编辑器，将结果插入列表顶部，不等待完整列表刷新。
- 编辑、置顶和归档可以使用乐观更新，失败时必须回滚并显示明确错误。
- Today 任务可以在右栏直接添加和勾选；完成任务保留在当天列表底部，避免操作后突然消失。
- Today 快速添加的任务默认使用 workspace 的“今天”；无日期任务属于 Inbox，不会永久堆积在 Today。
- 笔记中的 Markdown checkbox 不与任务表双向同步；首版两者保持清晰边界。
...
首版不提供注册、邀请、workspace 创建或切换 UI。保留这些表的目的只是让 Notes、Tasks、Library 等领域从一开始拥有明确租户边界，而不是提前实现完整多租户产品。未来每位好友可以拥有自己的个人 workspace，也可以通过 membership 加入共享 workspace。
...
- Today 快速添加默认将 workspace 当前日期写入 `due_date`。`due_date` 为空表示 Inbox；未完成且日期早于今天的任务也进入 Today，并标记为逾期。
...
- Markdown task list 仍属于笔记内容，不与 `tasks` 自动同步。

### 5.6 Workspace 隔离约束

- 认证中间件从 session 解析 `user_id` 和 `active_workspace_id`，生成不可由客户端直接伪造的 `RequestContext`。
- Note、Task 和未来 Library/RSS 服务的方法都必须显式接收 `workspace_id`；不得提供无 workspace 条件的列表、更新或删除方法。
...
- `scope=today` 返回逾期或计划在今天的未完成任务，以及按 workspace 时区计算为今天完成的任务；重点优先，其次按日期、可选时间、`sort_key` 和创建时间排序。
- `SessionInfo.workspace.today` 是服务端按 workspace 时区计算的 `YYYY-MM-DD`；Today 快速添加显式提交该日期，workspace 日期跨日或页面重新获得焦点时刷新会话信息。无日期任务属于 Inbox。
...
用户已从三套桌面端视觉探索中选定“双轨工作台”作为首版基线。它不把 Todo 和 Notes 融成一种数据，而是在同一个日常工作面中并列呈现两条最高频工作流：中央负责快速记录和回看，右侧负责当天行动。

![双轨工作台桌面端设计稿](./assets/dual-track-workbench-desktop.png)

桌面端结构固定为：

- **左侧导航约 208–224px**：品牌、Today、Notes、Tasks、Library、Reader、Chat、Search 和 Settings。当前模块用低饱和强调底色标识。
- **中央 Notes 主工作区**：顶部是可立即输入的 Markdown 编辑器，下方是按日期分组的笔记时间流。正文优先，元数据与操作降级。
- **右侧 Today 任务栏约 320–360px**：按重点和常规分组显示当天任务，支持勾选、时间和快速添加。
- **全局搜索**：从左侧入口或 `Ctrl/Cmd + K` 打开，不额外占用主工作区。

该方向的关键取舍：

- Notes 仍是页面主角，Todo 常驻但不抢占正文宽度。
- 左侧导航确保未来知识库、RSS 和 Chat 可以直接切换，不需要重做壳层。
- 主要内容使用列表和分隔线，不给每条笔记和任务套独立卡片。
- Today 右栏在宽屏常驻；窗口较窄时改为抽屉，不压缩 Notes 到不可读宽度。
...
- Notes 保持轻和快；Library 更偏编辑阅读；Tasks 更紧凑；Reader 更沉浸；Chat 更专注于来源与行动。
...
1. **记录**：编辑器是首页最重要的主操作。
2. **行动**：Today 任务常驻右栏，添加和完成不需要切换页面。
...
1. 编辑器获得焦点时轻微展开，保存成功后新笔记从编辑器下方进入时间流。
...
- `/`：编辑器、搜索、标签筛选、正常笔记时间流和 Today 任务右栏。
- `/tasks`：Phase 0B 提供全部未完成和已完成任务的简洁列表，不实现项目视图。
- `/archive`：归档笔记列表，可恢复或删除。
...
- 导航区负责模块切换、Inbox 数量和全局入口，不放模块内部复杂筛选。
- 主工作区是页面唯一视觉中心，Notes 和阅读页保持适合阅读的窄宽度。
- Notes 页面右栏固定显示 Today；Tasks 页面右栏显示任务详情；Library/Reader 显示来源；Chat 显示引用。没有必要时移除右栏。
- 页面顶部不设置大型营销式标题，只显示页面名称、范围、同步状态和必要操作。
- Command/Search 使用覆盖层，不长期占据宽屏空间。

Phase 0A 即实现左侧导航、Notes 主工作区和 Today 右栏，未来模块复用同一壳层，避免再做一次全局布局迁移。
...
- `noteList`：列表、分页、过滤条件和加载状态。
- `todayTasks`：当天任务、完成状态、分组和快速添加状态。
- `composer`：新建内容和提交状态。
...
- **笔记列表**：正文是视觉主体，时间、标签和操作处于次级层；操作在 hover、focus-within 或触控菜单中出现。
...
| Notes | 编辑器 + 时间流 | 正文、时间和快速记录 | 每条笔记套独立重卡片 |
...
- 第一屏内能看到品牌、编辑器和至少一条笔记，不出现欢迎 Hero 或数据卡片墙。
...
长期产品不是把笔记、书签、任务、RSS 和聊天五个应用并排放置，而是一个自托管的个人信息与行动中枢：统一收集想法、网页、订阅和任务，并让 Agent 基于这些真实资料帮助检索、总结与行动。
...
> RSS 负责发现，Library 负责沉淀，Notes 负责思考，Tasks 负责行动，Agent 负责连接。
...
Inbox 是连接各模块的关键，不是另一个内容类型。采集进入 Inbox 后，用户可以将其转为笔记、知识条目或任务。

### 8.3 数据边界：共享对象层 + 独立领域表

笔记、文章、任务、RSS 条目和对话的生命周期不同，不能全部塞进一张带 `type` 和任意 JSON payload 的万能表。推荐在出现第二种内容实体时引入共享对象层：
...
Notes + Today Tasks MVP 不需要提前建立完整对象层；在加入 Library 时通过数据库迁移引入，避免远期架构拖慢首日交付。
...
`source_object_id` 可以指向笔记、知识条目或 RSS 条目，从而支持从上下文创建任务，并从任务回看原始资料。

Phase 0A 只实现 Today 右栏，Phase 0B 补充简洁的全部任务页。Upcoming、Completed 分页、项目和来源关联进入后续增强；不复制复杂日历、习惯、番茄钟或高级重复规则。Markdown task list 继续属于笔记内容；系统未来可以提供“转换为任务”，但不做 Markdown checkbox 与任务表的双向实时同步。
...
- 记录请求 ID、路由、状态码和耗时，但不记录密码、Cookie 或完整笔记正文。
...
| A2 | 90 分钟 | Note 创建、列表、编辑、删除和基础搜索纵向切片 | 浏览器可完成笔记闭环，刷新和重启后数据存在 |
| A3 | 90 分钟 | Task 创建、Today 查询、完成、恢复、删除纵向切片 | Today 默认日期、逾期和 workspace 时区测试通过 |
| A4 | 120 分钟 | 双轨工作台：导航、编辑器、时间流、Today 右栏 | 选定桌面设计的结构和核心状态完整，不要求像素级精修 |
| A5 | 60 分钟 | Markdown 清洗、错误处理、SPA fallback、静态嵌入 | release 二进制可独立启动，直接刷新页面路由正常 |
| A6 | 30 分钟 | 冒烟测试与个人数据目录启动 | 核心验收清单通过，创建第一条真实笔记和任务 |

Phase 0A 退出条件：

- 登录后 10 秒内可以记录一条笔记或添加一条 Today 任务。
- Note 与 Task 的创建、读取、更新、删除均经过真实 SQLite 和 HTTP API，不使用 mock 数据。
...
4. 仅保留必要的自动化测试：认证、workspace 隔离、Note/Task 主链路、Today 日期边界和 Markdown 清洗。

不得削减：认证、Note CRUD、Today Task CRUD、SQLite 持久化、基础搜索、静态嵌入、生产数据目录隔离和桌面端可用性。
...
| Phase 4 | 候选 | 跨模块对象、搜索、关系与引用 | Notes/Library/Tasks/Reader 稳定 |
...
- 交付：认证、workspace 边界、Note CRUD、Today Task CRUD、基础搜索、Markdown 安全渲染、双轨工作台、静态嵌入。
...
- 交付：Inbox、Today、Upcoming、Completed、简单项目、手工排序，以及从 Note/Library 条目单向创建任务。
...
- 退出条件：Notes、Library、Tasks 和已保存 RSS 可从一个入口找回，搜索结果具备类型、来源和匹配片段。
...
- 第二切片：总结用户选定资料、查询任务、草拟 Note/Task。
...
| D-003 | 2026-08-23 | 产品首要入口是 Notes 中央工作区 + Today 右栏的双轨工作台 | 已接受 |
...
| 0.2 | 2026-08-23 | 扩展为 Notes、Library、Tasks、Reader、Agent 的长期产品蓝图 |
